...
EditorModel &EditorView::model() noexcept
{
    return model_;
}
const EditorModel &EditorView::model() const noexcept
{
    return model_;
}
...
std::expected<void, std::string> EditorView::load_file(const QString &path)
{
    ...
    {
        return std::unexpected("The file is not valid UTF-8 text.");
    }
    model_.load(text);
    document_changed();
    return {};
...
void EditorView::paintEvent(QPaintEvent *)
{
    ...
    for (std::size_t line = first; line < model_.line_index().line_count() && line < first + count; ++line)
    {
        ...
        const std::size_t selection_end =
            std::min(range.start.value + range.length.value, selected.start.value + selected.length.value);
...
void EditorView::keyPressEvent(QKeyEvent *event)
{
    ...
    else if (!event->text().isEmpty() && !event->modifiers().testFlag(Qt::ControlModifier) &&
...
QVariant EditorView::inputMethodQuery(Qt::InputMethodQuery query) const
{
    ...
    case Qt::ImSurroundingText:
        return qt::to_qstring(model_.document().to_string());
    case Qt::ImCursorPosition:
        return qt::utf16_offset_for_byte(model_.document().to_string(), model_.selection().cursor.value);
    case Qt::ImAnchorPosition:
        return qt::utf16_offset_for_byte(model_.document().to_string(), model_.selection().anchor.value);
    default:
        return QAbstractScrollArea::inputMethodQuery(query);
    }
}

int EditorView::line_height() const
{
...
void EditorView::update_scrollbars()
{
    ...
    verticalScrollBar()->setRange(
        0, std::max(0, static_cast<int>(model_.line_index().line_count()) * height - viewport()->height()));
