...
{
    return piece.buffer_start.value <= buffer.size()
        && piece.length.value <= buffer.size() - piece.buffer_start.value;
}
...
{
    if (!original_.empty()) {
        pieces_.push_back(Piece{BufferSource::original, ByteOffset{0}, ByteLength{original_.size()}});
    }
    validate();
}
...
void PieceTable::insert(ByteOffset position, std::string_view text)
{
    ...
    EDITOR_CHECK(text.size() <= std::numeric_limits<std::size_t>::max() - added_.size(), "add buffer overflow");
    EDITOR_CHECK(text.size() <= std::numeric_limits<std::size_t>::max() - size_.value, "document size overflow");

    const Piece inserted{BufferSource::added, ByteOffset{added_.size()}, ByteLength{text.size()}};
    added_.append(text);
...
void PieceTable::erase(ByteRange range)
{
    ...
    for (const Piece piece : pieces_) {
        const std::size_t piece_end = document_position + piece.length.value;
        const std::size_t kept_prefix = range.start.value > document_position
            ? std::min(range.start.value, piece_end) - document_position
            : 0;
        const std::size_t suffix_start = range_end < piece_end
            ? std::max(range_end, document_position) - document_position
            : piece.length.value;
...
void PieceTable::for_each_chunk(
    ByteRange range,
...
void PieceTable::validate_pieces(const std::vector<Piece>& pieces, ByteLength size) const
{
    ...
    for (std::size_t index = 0; index < pieces.size(); ++index) {
        ...
        EDITOR_CHECK(piece.length.value <= std::numeric_limits<std::size_t>::max() - accumulated, "piece lengths overflow");
...
{
    return source == BufferSource::original ? original_ : added_;
}
