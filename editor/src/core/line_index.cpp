...
void LineIndex::rebuild(const PieceTable &document)
{
    ...
    document.for_each_chunk(ByteRange{ByteOffset{0}, document_size_},
...
std::size_t LineIndex::line_of(ByteOffset offset) const
{
    ...
    return static_cast<std::size_t>(std::upper_bound(starts_.begin(), starts_.end(), offset,
                                                     [](ByteOffset value, ByteOffset start)
...
ByteRange LineIndex::line_range(std::size_t line) const
{
    ...
    const std::size_t end = line + 1 < starts_.size() ? starts_[line + 1].value - 1 : document_size_.value;
